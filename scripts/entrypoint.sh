...
NGINX_EOF
    ...
    if [ "$UPSTREAM" = "zeroclaw" ] && [ "${ZEROCLAW_ROOT_MODE:-}" != "1" ]; then
        ...
        exec su -s /bin/bash --whitelist-environment=HOME,SHELL,UPSTREAM,OPENCLAW_STATE_DIR,OPENCLAW_WORKSPACE_DIR,OPENCLAW_EXTERNAL_GATEWAY_PORT,OPENCLAW_INTERNAL_GATEWAY_PORT,OPENCLAW_GATEWAY_TOKEN,AUTH_USERNAME,AUTH_PASSWORD,OPENCLAW_CONTROL_UI_ALLOWED_ORIGINS,OPENCLAW_CONTROL_UI_ALLOW_INSECURE_AUTH,OPENCLAW_GATEWAY_BIND,OPENCLAW_PRIMARY_MODEL,OPENCLAW_FALLBACK_MODELS,OPENCLAW_IMAGE_MODEL_PRIMARY,OPENCLAW_IMAGE_MODEL_FALLBACKS,BROWSER_CDP_URL,BROWSER_DEFAULT_PROFILE,BROWSER_EVALUATE_ENABLED,BROWSER_SNAPSHOT_MODE,BROWSER_REMOTE_TIMEOUT_MS,BROWSER_REMOTE_HANDSHAKE_TIMEOUT_MS,WHATSAPP_ENABLED,WHATSAPP_DM_POLICY,WHATSAPP_ALLOW_FROM,WHATSAPP_GROUP_POLICY,WHATSAPP_GROUP_ALLOW_FROM,WHATSAPP_SELF_CHAT_MODE,WHATSAPP_MEDIA_MAX_MB,WHATSAPP_HISTORY_LIMIT,TELEGRAM_BOT_TOKEN,TELEGRAM_DM_POLICY,TELEGRAM_ALLOW_FROM,TELEGRAM_GROUP_POLICY,TELEGRAM_GROUP_ALLOW_FROM,DISCORD_BOT_TOKEN,DISCORD_DM_POLICY,DISCORD_DM_ALLOW_FROM,DISCORD_GROUP_POLICY,SLACK_BOT_TOKEN,SLACK_APP_TOKEN,SLACK_DM_POLICY,SLACK_GROUP_POLICY,HOOKS_ENABLED,HOOKS_TOKEN,HOOKS_PATH,ANTHROPIC_API_KEY,OPENAI_API_KEY,OPENROUTER_API_KEY,GEMINI_API_KEY,XAI_API_KEY,GROQ_API_KEY,MISTRAL_API_KEY,CEREBRAS_API_KEY,MOONSHOT_API_KEY,KIMI_API_KEY,KIMI_CODE_API_KEY,ZAI_API_KEY,OPENCODE_API_KEY,COPILOT_GITHUB_TOKEN,XIAOMI_API_KEY,VENICE_API_KEY,MINIMAX_API_KEY,AI_GATEWAY_API_KEY,SYNTHETIC_API_KEY,ZEROCLAW_API_KEY,COMPOSIO_API_KEY,AWS_ACCESS_KEY_ID,AWS_SECRET_ACCESS_KEY,AWS_REGION,AWS_SESSION_TOKEN,BEDROCK_PROVIDER_FILTER,OLLAMA_BASE_URL,DEEPGRAM_API_KEY,OP_SERVICE_ACCOUNT_TOKEN,GOG_KEYRING_PASSWORD,ZEROCLAW_PROVIDER,ZEROCLAW_MODEL,ZEROCLAW_WORKSPACE,ZEROCLAW_TEMPERATURE,ZEROCLAW_GATEWAY_HOST,ZEROCLAW_WHATSAPP_APP_SECRET "$UPSTREAM" -c 'cd /data && /app/scripts/entrypoint.sh'
    fi
fi
