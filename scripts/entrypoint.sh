...
NGINX_EOF
    ...
    if [ "$UPSTREAM" = "zeroclaw" ] && [ "${ZEROCLAW_ROOT_MODE:-}" != "1" ]; then
        ...
        exec "$0" "$@"
    elif [ "$UPSTREAM" != "zeroclaw" ] && [ "${SUPERVISOR_ROOT_MODE:-}" != "1" ]; then
        log_info "Running supervisord as root for $UPSTREAM (user= in program sections)..."
        export SUPERVISOR_ROOT_MODE=1
        exec "$0" "$@"
    fi
fi
