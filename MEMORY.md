...
ZeroClaw requires a **complete TOML configuration** matching the output of `zeroclaw onboard`. The config is generated by `scripts/configure-zeroclaw.js`.
...
max_actions_per_hour = 20  # REQUIRED!
...
[channels_config]
cli = true  # MUST be boolean, NOT [channels_config.cli] table!
...
# ... many more required sections
```

**Common errors:**

1. **"missing field `cli`"**: `[channels_config]` must have `cli = true` as a boolean field, NOT a nested `[channels_config.cli]` table with `enabled = true`.

2. **"missing field `max_actions_per_hour`"**: The `[autonomy]` section requires this field.

3. **Config too small**: If config is ~594 bytes, it's the OLD format. Valid config is ~5KB+.

**To generate valid config manually:**
```bash
zeroclaw onboard --api-key sk-your-key --force
```

**Never run `zeroclaw doctor`** - it rewrites config incorrectly by adding incomplete `[channels_config]` sections.

Reference: https://github.com/zeroclaw-labs/zeroclaw?tab=readme-ov-file#configuration
...
### Sticky Disk Key Must Include Run ID

The workflow uses `useblacksmith/stickydisk@v1` to pass Docker images between jobs. The sticky disk key MUST include `github.run_id` to prevent loading stale images from previous runs:

```yaml
# CORRECT - includes run_id
key: ${{ github.repository }}-${{ matrix.upstream }}-docker-image-${{ github.run_id }}

# WRONG - persists across runs, loads stale images
key: ${{ github.repository }}-${{ matrix.upstream }}-docker-image
```

Without `run_id`, the smoke test job may load an old Docker image from a previous CI run, causing confusing failures where the config file doesn't match the expected format.

### Dockerfile Cache Busting for Scripts

When `scripts/configure-zeroclaw.js` changes, the Docker layer cache must be invalidated. Copy the specific file first:

```dockerfile
# Copy configure-zeroclaw.js first to bust cache when it changes
COPY --chown=${UPSTREAM}:${UPSTREAM} scripts/configure-zeroclaw.js /app/scripts/configure-zeroclaw.js
COPY --chown=${UPSTREAM}:${UPSTREAM} scripts/ /app/scripts/
```

This ensures that changes to the config generator are picked up in the built image.
